...
class DataFetcher:
    ...
    def get_500etf_price(self, start_date, end_date):
        """
        获取中证500ETF的价格数据
    ...
    def get_atm_options(self, trade_date):
        """
        获取指定交易日的平值期权
        ...
        """
        # 获取期权链
        option_chain = self.get_option_chain(trade_date)
    ...
    def prepare_backtest_data(self, start_date, end_date):
        ...
        """
        # 获取ETF价格数据
        etf_data = self.get_500etf_price(start_date, end_date)

        # 获取期间内的所有交易日
        trade_dates = etf_data['trade_date'].dt.strftime('%Y%m%d').tolist()

        # 获取每个交易日的平值期权数据
        all_options = []
        for date in trade_dates:
            try:
                options = self.get_atm_options(date)
                if not options.empty:
                    all_options.append(options)
            except Exception as e:
                print(f"获取{date}的期权数据失败: {e}")

        if all_options:
            option_data = pd.concat(all_options, ignore_index=True)
            return etf_data, option_data
        else:
            return etf_data, pd.DataFrame()
